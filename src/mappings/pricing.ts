...
export function getEthPriceInUSD(uniswapFactoryContract: UniswapFactoryContract): BigDecimal {
    let pairAddress = uniswapFactoryContract.getPair(Address.fromString(STABLE_TOKEN), Address.fromString(WETH_ADDRESS));
    if (pairAddress.toHexString() == ADDRESS_ZERO) return ZERO_BD;
    return getRelativePrice(pairAddress, WETH_ADDRESS);
}

export function findEthPerToken(uniswapFactoryContract: UniswapFactoryContract, token: Token): BigDecimal {
