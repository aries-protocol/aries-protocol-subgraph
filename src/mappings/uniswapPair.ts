...
import { UniswapRouter as UniswapRouterContract } from "../types/Factory/UniswapRouter";
import { Pair as PairContract, Sync as Sync1 } from "../types/templates/Pair/Pair";
import { VaultToken as VaultTokenContract, Sync as Sync2 } from "../types/templates/VaultToken/VaultToken";
import { getEthPriceInUSD, findEthPerToken } from "./pricing";
import { convertTokenToDecimal, ONE_BI, ZERO_BD, BI_18, ZERO_BI } from "./helpers";
...
export function handleSync1(event: Sync1): void {
    ...
    _handleSync(uniswapFactory, pair, reserves.value0, reserves.value1, totalSupply);
}
...
function _handleSync(
  ...
  */
    ...
    // update LP price
    if (pair.totalSupply.notEqual(ZERO_BD)) pair.derivedETH = pair.reserveETH.div(pair.totalSupply);
    else pair.derivedETH = ZERO_BD;
    if (pair.totalSupply.notEqual(ZERO_BD)) pair.derivedUSD = pair.reserveUSD.div(pair.totalSupply);
